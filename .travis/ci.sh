docker build -t jefftian/face-swap .
docker images
docker run --network host -e CI=true -d -p 127.0.0.1:5000:5000 --name face-swap jefftian/face-swap
docker ps | grep -q face-swap
docker ps -aqf "name=face-swap$"
docker push jefftian/face-swap
